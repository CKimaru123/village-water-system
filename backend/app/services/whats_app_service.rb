...
class WhatsAppService
  API_VERSION     = ENV.fetch('WHATSAPP_API_VERSION', 'v25.0')
  ...
  # Send a plain-text WhatsApp message.
  #   to:   phone number in E.164 format without '+' (e.g. "254704363704")
  #   body: message text (max 1024 chars)
  def self.send_message(to:, body:)
    return unless configured?

    # Strip leading + if present
    phone = to.to_s.gsub(/\A\+/, '')
    ...
    Rails.logger.error "[WhatsAppService] send_message failed: #{e.message}"
    nil
  end
end
